...
module.exports = (client) => {
    ...
    client.on('messageCreate', async message => {
        ...
        if (cleanContent === `${prefix}backup` || cleanContent.startsWith(`${prefix}backup`) || cleanContent === '.backup' || cleanContent.startsWith('.backup')) {
            ...
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        ...
                        `• **Channels Saved:** \`${backupData.summary.totalChannels}\`\n` +
                        `• **Messages Archived:** \`${backupData.summary.totalArchivedMessages}\` msgs\n\n` +
                        `*Run \`${prefix}restore\` if the server is ever nuked.*`
                    )
                    .setTimestamp();
        ...
        if (cleanContent === `${prefix}restore` || cleanContent.startsWith(`${prefix}restore`) || cleanContent === '.restore' || cleanContent.startsWith('.restore')) {
            if (!isAdmin && !isOwner) return message.reply('❌ Admins or Server Owner only.');
            ...
            if (!backup) return message.reply(`❌ No backup found for this server! Run \`${prefix}backup\` first.`);
            ...
            let confirmation;
            try {
                confirmation = await response.awaitMessageComponent({ filter, time: ONE_YEAR_MS });
            } catch (e) {
