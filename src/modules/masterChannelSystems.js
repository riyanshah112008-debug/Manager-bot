...
async function provisionMasterServerStructure(interaction, client, ownerPrompt) {
    ...
    // Open a Ticket & Staff Application Channel
...
function registerSystemListeners(client) {
    ...
    // E. Staff Application Modal
    client.on('interactionCreate', async (interaction) => {
        if (!interaction.isButton() || interaction.customId !== 'sys_apply_staff') return;
...
async function handlePolicyVoteButtons(interaction) {
    ...
    // CODACY FIX: String object query isolation
    const msgIdStr = String(interaction.message ? interaction.message.id : '');
    if (!msgIdStr) return;

    const voteQuery = { messageId: msgIdStr };
    const voteDoc = await PolicyVote.findOne(voteQuery);
    if (!voteDoc) return;
