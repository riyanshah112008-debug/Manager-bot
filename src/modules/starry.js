...
module.exports = (client) => {
    ...
    client.sendPremiumModDM = async (member, moderator, action, reason, duration, guild, caseId = 'N/A', appealLink = null) => {
        ...
        if (!isGuildPremium) {
            const basicEmbed = new EmbedBuilder()
                ...
                .addFields({ name: 'Action', value: actionType.toUpperCase(), inline: true }, { name: 'Reason', value: reason || 'No reason provided.', inline: true })
                .setFooter({ text: `${guild.name} • Upgrade server to Premium for enhanced notices.` }).setTimestamp();
        ...
        const modEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '👤 Moderator', value: `\`${moderator.user.username}\``, inline: true }, { name: '🛡️ Action', value: `\`${actionType.charAt(0).toUpperCase() + actionType.slice(1)}\``, inline: true },
                { name: '🏷️ Case ID', value: `\`#${caseId}\``, inline: true }, { name: '📝 Reason for Action', value: `>>> ${reason || 'No specific reason was provided.'}`, inline: false },
                { name: '⏳ Duration', value: durationDisplay, inline: true }, { name: '📅 Time of Action', value: `<t:${Math.floor(Date.now() / 1000)}:f>`, inline: true }
            )
            .setThumbnail(guild.iconURL({ dynamic: true, size: 512 }))
            .setFooter({ text: `💎 Premium Automated Notice`, iconURL: client.user.displayAvatarURL() }).setTimestamp();
        ...
        try { await member.send({ embeds: [modEmbed], components: components }); return true; } catch (error) { return false; }
    };
    client.on('messageCreate', async (message) => {
        ...
        if (text === '.dev') {
            ...
            const devEmbed = new EmbedBuilder().setColor('#2C2F33').setTitle('💻 Starry Developer Menu').setDescription('**Owner-Only Text Commands:**\n\n` .servers ` - Lists all servers.\n` .serverdump ` - Full text data dump.\n` .sysinfo ` - Bot stats.\n` .eval <code> ` - Run raw JavaScript.\n` .broadcast <msg> ` - Send message to ALL servers.\n` .leaveserver <ID> ` - Remotely force leave.\n` .blacklist <ID> ` - Block a user.\n` .emergencyleave ` - Force leave current server.\n` .restart ` - Kills the bot process.\n` .setstatus <text> ` - Changes status.').setFooter({ text: 'Starry Developer CLI' });
            try { await message.author.send({ embeds: [devEmbed] }); return message.reply('📬 Sent the developer CLI guide to your DMs!').catch(() => {}); } catch (err) { return message.reply('❌ I couldn\'t DM you!').catch(() => {}); }
        }
        if (text === '.sysinfo') {
            if (!isOwner) return message.reply(notOwnerMsg).catch(()=>{});
        ...
        if (text.startsWith('.blacklist ')) {
            ...
            if (blacklistedUsers.has(targetId)) { blacklistedUsers.delete(targetId); return message.reply(`✅ Removed \`${targetId}\` from the blacklist.`).catch(()=>{}); } 
            else { blacklistedUsers.add(targetId); return message.reply(`🚫 Added \`${targetId}\` to the blacklist.`).catch(()=>{}); }
        }
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.isChatInputCommand() && interaction.commandName === 'devpanel') {
            const embed = new EmbedBuilder().setTitle('💻 Starry Developer Control Panel').setDescription('Select an operation below.').setColor('#5865F2');
            ...
            const row4 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dev_restart').setLabel('Reboot Bot Process').setStyle(ButtonStyle.Danger).setEmoji('🔄'));
        ...
        if (interaction.isButton() && interaction.customId.startsWith('dev_')) {
            ...
            if (id === 'dev_sysinfo') return interaction.reply({ content: `📊 **System Info:**\n- **RAM:** ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB\n- **Ping:** ${client.ws.ping}ms`, ephemeral: true });
            if (id === 'dev_servers') {
            ...
            if (id === 'dev_dump') return interaction.reply({ content: 'Use the text command `.serverdump` instead!', ephemeral: true });
            if (id === 'dev_restart') { await interaction.reply({ content: '🔄 **Rebooting...**', ephemeral: true }); process.exit(1); }
            if (id === 'dev_emergencyleave') { 
                if (!interaction.guild) return interaction.reply({ content: '❌ Not in a server!', ephemeral: true }); 
                await interaction.reply({ content: 'Leaving. 👋', ephemeral: true }); return interaction.guild.leave(); 
            }
        ...
        if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_')) {
            ...
            if (id === 'modal_broadcast') { 
                await interaction.deferReply({ ephemeral: true }); 
                const msg = interaction.fields.getTextInputValue('broadcast_msg'); let count = 0; 
                client.guilds.cache.forEach(guild => { 
            ...
            if (id === 'modal_blacklist') { 
                ...
                if (blacklistedUsers.has(targetId)) { blacklistedUsers.delete(targetId); return interaction.reply({ content: `✅ Removed \`${targetId}\` from blacklist.`, ephemeral: true }); } 
                else { blacklistedUsers.add(targetId); return interaction.reply({ content: `🚫 Added \`${targetId}\` to blacklist.`, ephemeral: true }); } 
            }
            if (id === 'modal_leave') { 
                ...
                await guildToLeave.leave(); return interaction.reply({ content: `✅ Successfully left **${guildToLeave.name}**.`, ephemeral: true }); 
            }
        }
    }); // 🛑 This correctly closes the Interaction Engine!
    // ==========================================
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (message.guild) {
            try {
                ...
                if (settings && settings.triggerWord) { triggerWord = settings.triggerWord.toLowerCase(); displayName = settings.triggerWord; }
            } catch (err) { }
        }
        ...
        console.log(`[Starry AI] Waking up for message from ${message.author.username}...`);
        ...
        if (isImageRequest) {
            ...
            } catch (error) { return replyMsg.edit('❌ I had trouble drawing that. Try a simpler prompt.').catch(() => {}); }
        }
        ...
        try {
            console.log(`[Starry AI] Calling Gemini API...`);
            ...
            console.log(`[Starry AI] Response generated successfully.`);
            ...
            if (cmdMatch) {
                ...
                if (action === 'CLEAR') { 
                    ...
                    if (isNaN(rawAmount)) { const match = (cmdMatch[2] || '').match(/\d+/); rawAmount = match ? parseInt(match[0]) : 0; }
            ...
            if (functionName) {
                ...
                if (['give_role', 'remove_role', 'create_role', 'delete_role'].includes(functionName)) {
                    ...
                    if (functionName === 'create_role') { 
                        ...
                        const newRole = await message.guild.roles.create({ name: args.roleName || 'New Role', color: roleColor }); 
                ...
                if (functionName === "unban_member" && hasPerm(PermissionFlagsBits.BanMembers)) { await message.guild.members.unban(tId).catch(()=>{}); return message.reply("✅ User Unbanned."); }
                ...
                if (functionName === "untimeout_member" && hasPerm(PermissionFlagsBits.ModerateMembers)) { await tMember.timeout(null).catch(()=>{}); return message.reply(`✅ Removed timeout from <@${tId}>.`); }
            ...
            if (error.status === 429) return message.reply("⏳ **I'm taking a quick breather!** We hit the free-tier rate limit. Try again in a minute!").catch(console.error);
